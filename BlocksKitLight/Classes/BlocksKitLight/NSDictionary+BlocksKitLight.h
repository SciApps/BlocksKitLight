 ...
 */
@interface BKL_GENERICS(NSDictionary, KeyType, ObjectType) (BlocksKitLight)
...
/** Enumerates through the dictionary concurrently and executes
 ...
 @param block A block that performs an action using a key/value pair.
 */
- (void)bkl_apply:(void (^)(KeyType key, ObjectType obj))block;
 ...
 */
- (NSDictionary *)bkl_select:(BOOL (^)(KeyType key, ObjectType obj))block;
...
/** Loops through a dictionary to find the key/value pairs not matching the block.
 ...
 This is useful, as one may expect, for filtering objects.
	 NSDictionary *strings = [userData bkl_reject:^BOOL(id key, id value) {
 ...
 */
- (NSDictionary *)bkl_reject:(BOOL (^)(KeyType key, ObjectType obj))block;
 ...
 */
- (NSDictionary *)bkl_map:(id (^)(KeyType key, ObjectType obj))block;
