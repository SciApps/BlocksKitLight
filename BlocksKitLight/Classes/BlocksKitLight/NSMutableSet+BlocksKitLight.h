 ...
 */
@interface BKL_GENERICS(NSMutableSet, ObjectType) (BlocksKitLight)

/** Filters a mutable set to the objects matching the block.

 @param block A single-argument, BOOL-returning code block.
 @see <NSSet(BlocksKitLight)>bkl_reject:
 */
...
/** Filters a mutable set to all objects but the ones matching the block,
 ...
 @param block A single-argument, BOOL-returning code block.
 @see <NSSet(BlocksKitLight)>bkl_select:
 */
...
/** Transform the objects in the set to the results of the block.
 ...
 This is sometimes referred to as a transform, mutating one of each object:
	[controllers bkl_map:^id(id obj) {
	  return [obj view];
	}];

 @param block A single-argument, object-returning code block.
 @see <NSSet(BlocksKitLight)>bkl_map:
 */
